...
class HybridEncoder(nn.Module):
    """Combines Vision (CNN) and State (FC) processing"""
    def __init__(self):
        super().__init__()
        
        # 7x7x3 -> 800 features
        self.cnn = nn.Sequential(
            layer_init(nn.Conv2d(3, 16, 2, stride=1, padding=0)), nn.ReLU(),
            layer_init(nn.Conv2d(16, 32, 2, stride=1, padding=0)), nn.ReLU(),
            nn.Flatten(),
        )
        
        # [X, Y, HasKey] -> 64 features
        self.state_fc = nn.Sequential(
            layer_init(nn.Linear(3, 64)), nn.ReLU()
        )
...
class Agent(nn.Module):
    def __init__(self, action_dim, skill_dim):
        ...
        self.actor = nn.Sequential(
            layer_init(nn.Linear(self.encoder.feature_dim + skill_dim, 256)), nn.Tanh(),
...
class Discriminator(nn.Module):
    """Predicts Skill based ONLY on State (X, Y, Key)."""
